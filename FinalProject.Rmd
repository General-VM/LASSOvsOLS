...
ols_sq_error <- function(significant_coefs, input_data) {
  ...
  sig_predictors_matrix <- as.matrix(generated_data[, significant_predictors])
  res <- data.frame(Residuals = generated_data$Y - ols_beta_0 - sig_predictors_matrix %*% ols_coefs)
