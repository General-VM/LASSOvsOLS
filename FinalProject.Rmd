...
$$\hat\beta_{OLS}=argmin_\beta \frac{1}{2n} \|Y-X\beta\|^2_{2}$$ where - Y = $(Y_1, ..., Y_n)^T$ is n x 1 response vector - X = $(X_1, ..., X_n)^T$ is n x p design matrix of covariates - $\beta$ = $(Y_1, ..., Y_n)^T$ is p-dimensional regression coefficient vector
...
$$\hat\beta_{LASSO} = argmin_{\beta} \{ L(\beta)+\lambda\|\beta\|_1\}$$ $$\hat\beta_{LASSO} = argmin_\beta \{ \frac{1}{2n} \sum_{i=1}^{n}(Y_i-\sum_{j=1}^{p}X_{ij}\beta_j)^2+\lambda\sum_{j=1}^{p}|\beta_j|\}=argmin_{\beta}F(\beta)$$
...
2.  For each $\lambda\in\Lambda$, evaluate $\hat\beta^{-(h)}_{LASSO}(\lambda)$. the LASSO estimator based on the dataset $D\D_h$, h=1, ..., *m*. Each $D_h$ is treated as a *test dataset*, while $D\D_h$ as a *training dataset*.

3.  Thus, for each $\lambda\in\Lambda$ we get *h* LASSO estimators, making in total *h x q* LASSO estimators.

4.  Define the loss function

$$CV(\lambda)=\sum_{h=1}^{m}\sum_{i:(X_i,Y_i)\in D_h}(Y_i-X_i^T\hat\beta^{-(h)}_{LASSO}(\lambda))^2$$ 5. Choose $\lambda$ that minimized the loss function.

It can be seen that the OLS estimator can be derived from the LASSO estimator as t-\>$\infty$. It should also be noted that while LASSO can set coefficients to zero, there is the risk of over-shrinkage.
...
$$Y=\beta_0+X\beta+\epsilon$$ where - Y = $(Y_1, ..., Y_n)^T$ is n x 1 response vector - X = $(X_1, ..., X_n)^T$ is n x p design matrix of covariates - $\beta$ = $(Y_1, ..., Y_n)^T$ is p-dimensional regression coefficient vector - $\beta_0$ is the intercept - $\epsilon$ is the n-dimensional noise vector
...
## 2.2 Design of OLS Method

-   describe functions [?]

## 2.3 Design of LASSO Method
